from consts import SUCCESSFUL_CHAT_RESPONSE, TEST_FXA_TOKEN


def test_missing_auth(mocked_client):
...
def test_invalid_fxa_auth(mocked_client):
	response = mocked_client.post(
		"/v1/chat/completions",
...
def test_successful_request_with_mocked_fxa_auth(mocked_client):
	response = mocked_client.post(
		"/v1/chat/completions",
